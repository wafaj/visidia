...
def genGraph(NoNodes=100,proximity=300,Xmax=1000,Ymax=1000,sep=100,mat=[[True]],X1Comp=0,Y1Comp=0,X2Comp=1000,Y2Comp=1000):
	nodes = []
	compromisedNodeIds = []
	...
	for y in range(0,Ymax+1,sep):
		nodes += [[]]
		for x in range(0,Xmax+1,sep):
			if mat[int(y/RYmax)][int(x/RXmax)] == True:
...
if __name__ == '__main__':
	if(len(sys.argv) != 6):print("Usage: {} <NumberOfNodes> <Proximity> <MaximumX> <MaximumY> <<Grid Seperation>".format(sys.argv[0]))
	else: 
		args = [int(sys.argv[x]) for x in range(1,6)]
		...
		for x in range(rows):
			for y in range(cols):
				if mat[x][y] == '#':
					...
					mat[x][y] = False
		genGraph(args[0],args[1],args[2],args[3],args[4],mat)
