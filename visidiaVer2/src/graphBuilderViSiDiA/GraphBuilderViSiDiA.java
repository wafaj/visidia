				...
				.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent arg0) {
						if (windowBuildGraph.validInput()) {
							System.out
									.println("le bouton dans l'autre fenetre build graphe cliquï¿½ avec "
					...
					private boolean generateGraph() {
						if (windowBuildGraph.validInput()) {
							System.out
									.println("vous pouvez gï¿½nï¿½rer votre graphe");
							if (windowBuildGraph.getSelectedTopology().equals(
								...
								System.out
										.println("Gération du graphe réussit ");
								return true;
							} else {
								System.out
										.println("Gï¿½nï¿½ration du graphe echoué ");
								return false;
							}

						} else {
							System.out
									.println("vous ne pouvez gï¿½nï¿½rer le graphe, erreur de saisie");
							return false;
						}
					}
					...
					private boolean genereateRectangle(int maxRange, int nbNodes, int densite,
						...
						System.out.println("unitï¿½ =ceil(a)=" + unitee);
						int i = 0;
					...
					private boolean genereateGrille(int maxRange, int nbNodes) {
						...
						for (int i = 0; i < nbNodes; i++) {
							for (int j = i + 1; j < nbNodes; j++) {
								...
								//if ((graph.getVertex(i).distanceFrom(graph.getVertex(j))) <= maxRange) {
									//Edge edge = new Edge(null, null, false);
									//edge.setOriented(false);
									//edge.setOrigin(graph.getVertex(i));
									//edge.setDestination(graph.getVertex(j));						
							//		Edge edge = new Edge(graph.getVertex(i),graph.getVertex(j), false);
									graph.getVertex(i).linkTo(
											graph.getVertex(j), false);
								}
							}
						}
						// ////////////////////////*********************************//////////////////////////:
